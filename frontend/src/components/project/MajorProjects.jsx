...
const MajorProjects = () => {
    ...
    return (
        <div className="w-full relative mb-10">
            <div className="flex items-center justify-between mt-8 px-6 border-b border-white pb-4">
                ...
                {loggedIn && (
                    ...
                    >
                        {/* Corrected logic for button text based on modalIsOpen state */}
                        {modalIsOpen ? (
            ...
            {/* Display message if not loading and no projects found */}
            {!isLoading && topFour.length === 0 && (
                <p className="text-gray-500 text-center p-8">No recent works available. Add a project!</p>
            )}
            ...
            {/* Render projects only when not loading and projects exist */}
            {!isLoading && topFour.length > 0 && (
                <div className="w-full mt-8 flex flex-wrap gap-6 justify-center">
                    {/* The slice and reverse operations are now safe */}
